...
All diagrams are **pure Mermaid** — they render natively on GitHub, GitLab, VS Code preview (with a Mermaid extension), and at [mermaid.live](https://mermaid.live). Every diagram was extracted from the **live code and the deployed `factory_db` schema**, not from memory.

| Doc | Contents |
|---|---|
| [01 — Use Case Diagram](01-use-case-diagram.md) | Actors (CEO, Manager, Accountant, Procurement Officer, System), full use-case diagram, include/extend relations, per-use-case specifications, role → use-case matrix |
...
- To edit diagrams visually: copy a Mermaid block into [mermaid.live](https://mermaid.live) (ERD → "ER diagram" syntax is supported) or into draw.io via *Arrange ▸ Insert ▸ Advanced ▸ Mermaid* for fully editable shapes.
